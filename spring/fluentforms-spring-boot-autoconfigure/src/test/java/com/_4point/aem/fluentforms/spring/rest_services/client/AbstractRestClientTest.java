...
/**
 * This class contains tests for the RestClient class that are independent of the underlying implementation.
 * 
 * The tests in this class use WireMock to mock the AEM server responses, and verify that the RestClient behaves 
 * correctly in various scenarios (e.g. different response codes, content types, headers, etc.).
 * 
 * This class is extended by implementation-specific test classes (e.g. JerseyRestClientTest, SpringRestClientRestClientTest),
 * A copy of this class resides both in the Spring Boot autoconfiguration project and the Jersey client project,
 * to allow testing of both implementations without creating a dependency between the two projects.
 */
@WireMockTest
abstract class AbstractRestClientTest {
	...
	AbstractRestClientTest(Supplier<RestClientFactory> restClientFactorySupplier) {
	...
	void setup(WireMockRuntimeInfo wmRuntimeInfo) {
		...
		underTest = restClientFactorySupplier.get().apply(aemConfig, ENDPOINT, ()->CORRELATION_ID_TEXT);
	}
	...
	void testMultipleEndpoints() throws Exception {
		...
		var client1 = restClientFactorySupplier.get().apply(aemConfig, endPoint1, null);
		var client2 = restClientFactorySupplier.get().apply(aemConfig, endPoint2, ()->CORRELATION_ID_TEXT);
